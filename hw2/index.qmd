...
In a Scripts file, I defined the gini function.

Then I create a histogram of the population.
...
First I define what happens in a transaction, a random split.
...
Two random actors shall meet.

```{r}
anyone <- function(N) {
  sample(0:(N-1), 2)  # Randomly pick two different actors from the range 0 to N-1
}

# Example: Select two random actors from a population
N <- 5000  # Define the size of the population
anyone(N)
```

**Simulation**

A function, how to perform single steps in simulation is needed, then for T steps.
...
For many steps, another way to visualize is needed.
